...
def navigate_home_page_with_info():
    print("\n📱 Ana sayfaya gidiliyor...")
    disappearing_page.navigate_disappearing_elements()
    time.sleep(2)


def button_visible_test():
    try:
        for button_name in visible_buttons:
            try:
                print(f"\n🖱️  '{button_name}' butonu test ediliyor...")
...
def test_disappearing_elements_US009_TC01():
    driver_manager = DriverManagerPage()
    disappearing_page = DisappearingElementsPage()
    driver_manager.consol_info_first_step()
    disappearing_page.navigate_home_page_with_info()
    disappearing_page.button_visible_test()
    driver_manager.close_driver()
