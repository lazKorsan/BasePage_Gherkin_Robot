...
from pages.DisappearingElementsPage import DisappearingElementsPage, driver_manager


def consol_info_first_step():
    print("=" * 60)
    print("DİSAPPEARING ELEMENTS TESTİ BAŞLIYOR")
    print("=" * 60)


def navigate_home_page_with_info(driver_manager, disappearing_page):
...
def button_visible_test(disappearing_page, visible_buttons):
    try:
...
def test_disappearing_elements_US009_TC01():
    driver_manager = DriverManagerPage()
    disappearing_page = DisappearingElementsPage()

    consol_info_first_step()
    navigate_home_page_with_info(driver_manager, disappearing_page)
    ...
    # Butonların görünürlüğünü test et
    button_visible_test(disappearing_page, visible_buttons)


if __name__ == "__main__":
    test_disappearing_elements_US009_TC01()
